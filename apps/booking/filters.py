...
class PropertyCalenderDateFilter(filters.FilterSet):
    from_date = filters.DateFilter(field_name="date", lookup_expr="gte", required=True)
    to_date = filters.DateFilter(field_name="date", lookup_expr="lte", required=True)
    status = filters.ChoiceFilter(field_name="status", choices=CALENDAR_STATUS_CHOICES)
    ...
    def filter_queryset(self, queryset):
        ...
        if from_month not in {current_month, next_month}:
            raise ValidationError(_("Calendar range can start only in current or next months"))
        if from_month == current_month and from_date < today:
            raise ValidationError(_("For the current month, from_date must be today or later"))

        month_diff = (to_month.year - from_month.year) * 12 + (to_month.month - from_month.month)
        if month_diff > 1:
            raise ValidationError(_("Calendar range can cover at most current and next month"))
