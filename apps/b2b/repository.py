...
from apps.b2b.models import B2BUserRole, BudgetRequestStatus
...
def list_departments_with_budget(
    ...
    - ``used_amount`` / ``on_trip_amount`` – no spend source is tracked
      per-department anymore, so both are always ``0``.
    """
...
def list_employees_with_individual_limit(
    ...
    - ``used_amount`` / ``on_trip_amount`` – always 0; kept in the payload
      for API compatibility. No spend source is tracked per-employee.
    """
    where = [
        "e.company_id = %s",
        "e.is_active = TRUE",
...
def get_rule_used_amount(company_id: int, applies_to: str, target_id: int | None) -> Decimal:
    """Real money spent against a policy rule's scope.

    No spend source is tracked anymore, so this is always ``0``.
    """
    return Decimal("0")
...
def list_transactions(
    ...
    """Paginated transaction history for the analytics page, sourced from
    budget requests (`b2b_budget_request`)."""
...
def _approved_spend_for_range(company_id: int, start: datetime, end: datetime) -> Decimal:
    ...
    requests, matching the calculation used by ``get_dashboard_summary``."""
    row = fetch_one(
        f"""
    ...
    return row.get("approved_spend") or Decimal("0")
...
def get_spending_chart(company_id: int, period: str) -> dict[str, Any]:
    ...
    rows = fetch_all(
        ...
        ORDER BY d.bucket
        """,
        [company_id, interval, start, now, interval],
    )
...
def get_dashboard_summary(company_id: int) -> dict[str, Any]:
    ...
    - ``spent_this_month``       – sum of approved budget-request amounts
      for trips created this calendar month
    ...
    spent_row = fetch_one(
        ...
        JOIN {B2B_BUSINESS_TRIP_TABLE} bt ON bt.id = br.trip_id
        WHERE bt.company_id = %s AND br.status = 'approved'
          AND EXTRACT(YEAR FROM br.created_at) = %s
          AND EXTRACT(MONTH FROM br.created_at) = %s
        """,
...
def get_department_monthly_spending(
    ...
    ``month_spend`` = sum of approved budget-request amounts raised
    directly against the department, created inside the month.
