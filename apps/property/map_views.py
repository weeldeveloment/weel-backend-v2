...
ALL_KINDS = (PROPERTY_KIND_APARTMENT, PROPERTY_KIND_COTTAGE)
...
MAP_QUERY_PARAMS = PROPERTY_LIST_QUERY_PARAMS + [
    ...
    openapi.Parameter(
        ...
        description="Comma-separated property kinds: `apartment,cottage`. Repeatable.",
    ),
]
...
class PropertyMapCardsView(APIView):
    ...
    def _load_row(guid: str) -> dict[str, Any] | None:
        for loader in (get_apartment_for_public, get_cottage_for_public):
            try:
                row = loader(guid)
            except Exception:
                continue
...
class PropertySearchView(APIView):
    ...
    @swagger_auto_schema(
        ...
        operation_description=(
            "Mixed apartment / cottage search returning the compact card payload used on "
